...
import models
from models.resnet_cifar import resnet_cifar
from utils import *
...
def main():
	...
	if training_args.multicrop == True:
		trainset = get_multi_crop_dataset('./data', training_args)
	else:
	...
	if training_args.model == 'ResNet':
		...
		model = PyramidNet(dataset='cifar10', depth=training_args.depth, alpha=training_args.alpha, num_classes=10,
						   bottleneck=True, norm_layer=normlayer2d)
		...
		model = PyramidNetINV(dataset='cifar10', depth=training_args.depth, alpha=training_args.alpha, num_classes=10,
							  bottleneck=True)
	elif training_args.model == 'AdvPyramidNet':
		attacker = PGDAttacker(training_args.attack_iter, training_args.attack_epsilon, training_args.attack_step_size,
							   prob_start_from_clean=0.2 if not training_args.evaluate else 0.0)
		model = AdvPyramidNet(dataset='cifar10', depth=training_args.depth, alpha=training_args.alpha, num_classes=10,
							  bottleneck=True, attacker=attacker, writer=training_args.writer)
	elif training_args.model == 'resnet_cifar':
		attacker = PGDAttacker(training_args.attack_iter, training_args.attack_epsilon, training_args.attack_step_size,
							   prob_start_from_clean=0.2 if not training_args.evaluate else 0.0)
		model = models.__dict__[resnet_cifar](dataset='cifar10', depth=training_args.depth, alpha=training_args.alpha,
											  num_classes=10,
											  bottleneck=True, attacker=attacker, writer=training_args.writer)
		...
		model = PyramidNet_metaacon(dataset='cifar10', depth=training_args.depth, alpha=training_args.alpha,
	...
	if training_args.optim == 'adam':
		...
		optimizer = SAM(params, base_optimizer=torch.optim.SGD, rho=0.05, lr=training_args.lr, momentum=0.9,
